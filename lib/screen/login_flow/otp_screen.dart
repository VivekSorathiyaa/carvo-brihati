...
class _OtpScreenState extends State<OtpScreen> {
          ...
          children: [
            ...
            Padding(
              ...
              child: OutlinedButton(
                ...
                child: enableResend
                    ? Text('Resend Code',
                        style: TextStyle(color: onbordingBlue, fontSize: 15))
                    : Text('$second seconds',
                        style: TextStyle(color: onbordingBlue, fontSize: 15)),
              ),
            ),
              ...
              RichText(
                  ...
                  text: TextSpan(
                    children: [
                      TextSpan(
                          text:
                              'You need to enter 6-digit code that we have sent to your mobile number',
                          style: TextStyle(
                              fontFamily: FontFamily.europaWoff,
                              color: greyScale,
                              fontSize: 14)),
                      TextSpan(
                          text: ' ${widget.country} ${widget.mNumber}',
                          style: TextStyle(
                              fontFamily: FontFamily.europaBold,
                              color: notifire.getwhiteblackcolor,
                              fontSize: 15)),
  ...
  verifyOtp(String otp) async {
    try {
      ...
      var credential = await auth1.signInWithCredential(
          PhoneAuthProvider.credential(
              verificationId: widget.vId, smsCode: otp));

      if (credential.user != null) {
        try {
          await auth1.createUserWithEmailAndPassword(
            email: decode['email'],
            password: decode['password'],
          );

          await signup(decode['name'], decode['email'], decode['mobile'],
                  decode['password'], decode['ccode'])
              .then((value) {
        ...
        try {
          ...
          var credential = await auth1.signInWithCredential(
              PhoneAuthProvider.credential(
                  verificationId: widget.vId, smsCode: otp));
          if (credential.user != null) {
            signup(decode['name'], decode['email'], decode['mobile'],
                    decode['password'], decode['ccode'])
                .then((value) {
              if (value["ResponseCode"] == "200") {
                commonBottom();
