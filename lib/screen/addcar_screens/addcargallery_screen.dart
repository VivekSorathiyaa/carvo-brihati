...
class AddCarGalleryScreen extends StatefulWidget {
  ...
  final String? id;
  final String? uid;
  const AddCarGalleryScreen(
      {super.key, required this.title, this.id, this.carTitle, this.uid});
...
class _AddCarGalleryScreenState extends State<AddCarGalleryScreen> {
  ...
  Future addCar(carId, uid, size) async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('${Config.baseUrl}${Config.addGallery}'));
    ...
    if (response.statusCode == 200) {
      FirebaseAnalytics.instance
          .logEvent(name: "car_gallery_added", parameters: {
        "car_id": carId,
        'uid': uid,
      });
  ...
  Future editCar(uid, carId, recordId, img, size) async {
    var headers = {'Cookie': 'PHPSESSID=36pnj5phm83llglrqid2qnuquk'};
    var request = http.MultipartRequest(
        'POST', Uri.parse('${Config.baseUrl}${Config.editGallery}'));
    request.fields.addAll({
      'uid': uid,
      "car_id": carId,
      "record_id": recordId,
  ...
  Future listGallery(uId, carId) async {
    ...
    try {
      var response = await http.post(
          Uri.parse(Config.baseUrl + Config.galleryList),
          body: jsonEncode(body),
          headers: {
            'Content-Type': 'application/json',
          });
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          isLoading = false;
          editImage =
              galleryListmodal!.gallerylist[0].img.toString().split("\$;");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
            ...
            ? ElevatedButton(
                ...
                onPressed: () {
                  if (_formKey.currentState!.validate() && image.isNotEmpty) {
                    addCar(widget.id, widget.uid == "0" ? "0" : id['id'],
                ...
                : ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      if (size >= 1) {
                        ...
                        editCar(
                                widget.uid == "0" ? "0" : id['id'],
                                widget.id,
                                editId.toString(),
                                editImage.isEmpty ? '0' : editImage.join("\$;"),
                                image.length.toString())
                            .then((value) {
                          if (value['ResponseCode'] == "200") {
                            listCar(widget.uid == "0" ? "0" : id['id']);
                    ...
                    child: Text('Update Car Gallery'.tr,
                        style: const TextStyle(
                            color: Colors.white,
                            fontFamily: FontFamily.europaBold)),
                  ),
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: Text('${'Car Edit'.tr} ${widget.carTitle}',
            style: const TextStyle(
                color: Colors.white,
                fontFamily: FontFamily.europaBold,
                fontSize: 18)),
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                                  ...
                                  children: [
                                    Expanded(
                                      child: OutlinedButton(
                                        ...
                                        onPressed: () async {
                                          XFile? file = await picker1.pickImage(
                ...
                isvalidate
                    ? Text('Please Pick your Image.'.tr,
                        style: const TextStyle(color: Colors.red, fontSize: 13))
                    : const SizedBox(),
                Wrap(
                  spacing: 10.0,
                  children: [
                    for (int i = 0; i < editImage.length; i++)
                      Stack(
