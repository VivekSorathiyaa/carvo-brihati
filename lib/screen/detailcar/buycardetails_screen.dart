...
class _BuyCarDetailsScreenState extends State<BuyCarDetailsScreen> {
  ...
  Future bookRange(cId) async {
    ...
    try {
      var response = await http.post(
          Uri.parse(Config.baseUrl + Config.bookRange),
      ...
      var responseTwo = await http.post(
          Uri.parse(Config.baseUrl + Config.unavailableRange),
          body: jsonEncode(bodyTwo),
          headers: {
            'Content-Type': 'application/json',
          });
      ...
      if (response.statusCode == 200 && responseTwo.statusCode == 200) {
        ...
        BookRangeModal bookRangeData = bookRangeModalFromJson(response.body);
        final response2 = jsonDecode(responseTwo.body);
        final bookedDateList = response2['bookeddate'] as List;
        List<Bookeddate> unavailableDates = [];
        if (bookedDateList.isNotEmpty) {
          final unavailableDatesString =
              bookedDateList[0]['unavailable_dates'] as String;

          // Parse the string to a List<String>
          final parsedDates =
              (jsonDecode(unavailableDatesString) as List<dynamic>)
                  .map((date) => DateTime.parse(date as String))
                  .toList();

          // Convert parsed dates to Bookeddate model
          unavailableDates = parsedDates
              .map((date) => Bookeddate(pickupDate: date, returnDate: date))
              .toList();
          List<Bookeddate> combinedBookedDates = [
            ...bookRangeData.bookeddate,
            ...unavailableDates,
          ];

          bookRangeModal = BookRangeModal(
            bookeddate: combinedBookedDates,
            responseCode: bookRangeData.responseCode,
            result: bookRangeData.result,
            responseMsg: bookRangeData.responseMsg,
          );
          setState(() {
            bookRangeData = bookRangeModal;
            isLoading = false;
          });
        }

        // Combine the bookeddate lists from both responses
      } else {}
    } catch (e) {
      print(e);
    }
  ...
  //   print(body);
  //   try {
  //     var bookRangeResponse = await http.post(
  //       Uri.parse(Config.baseUrl + Config.bookRange),
  //       body: jsonEncode(body),
  //       headers: {
  //         'Content-Type': 'application/json',
  //       },
  //     );
  //    print(bookRangeResponse.body);
  //   var unavailableRangeResponse = await http.post(
  //     Uri.parse(Config.baseUrl + Config.unavailableRange),
  //     body: jsonEncode(body),
  //     headers: {
  //       'Content-Type': 'application/json',
  //     },
  //   );
  //   print(unavailableRangeResponse.body);
  // if (bookRangeResponse.statusCode == 200 &&
  //     unavailableRangeResponse.statusCode == 200) {
  //   SharedPreferences preferences = await SharedPreferences.getInstance();
  //   preferences.setBool('light', light);

  //   // Parse both responses into BookRangeModal objects
  //   BookRangeModal bookRangeData =
  //       bookRangeModalFromJson(bookRangeResponse.body);
  //   print(bookRangeData);
  //   BookRangeModal unavailableRangeData =
  //       bookRangeModalFromJson(unavailableRangeResponse.body);
  //   print(unavailableRangeData);
  //   // Combine the bookeddate lists from both responses
  //   List<BookRangeModal> combinedBookedDates = [];
  //   combinedBookedDates.add(bookRangeData);
  //   combinedBookedDates.add(unavailableRangeData);
  //   print("hii navod $combinedBookedDates");
  //   setState(() {
  //     // Update the bookeddate list in the current model
  //     //bookRangeModal = combinedBookedDates;

  //     // Now, you can use bookRangeModal which contains both sets of data
  //     isLoading = false;

  //     print("Combined Booked Dates: ${bookRangeModal.bookeddate}");
  //   });
  // } else {
  //   // Handle the error cases (e.g., invalid responses)
  // }
  // } catch (e) {
  //   print(e);
  // }
  ...
  date() {
    for (int i = 0; i < bookRangeModal.bookeddate.length; i++)
      for (int a = 0;
          a <=
              DateTime.parse(bookRangeModal.bookeddate[i].returnDate.toString())
                  .difference(DateTime.parse(
                      bookRangeModal.bookeddate[i].pickupDate.toString()))
                  .inDays;
          a++) {
        print(DateTime.parse(bookRangeModal.bookeddate[i].returnDate.toString())
            .add(Duration(days: a)));
      }
  }
            ...
            onPressed: () {
              ...
              if (sdate.toString() == "null" &&
                ...
                FirebaseAnalytics.instance
                    .logEvent(name: "book_car_dates", parameters: {
                  "id": widget.CarId,
                  "picktime": selectTime?.format(context) ?? "00:00",
                  "returnTime": selectTime1?.format(context) ?? "00:00",
                  "toggle": light,
                  ...
                  "sTime": '${date1.hour}:${date1.minute}:${date1.second}',
                  "eTime": '${date2.hour}:${date2.minute}:${date2.second}'
                });
                Navigator.push(
                    context,
                    MaterialPageRoute(
                ...
                print("***********");
                print(widget.minHrs);
                print(Duration(days: int.parse(widget.minHrs)));
                print(diff);
                print("***********");
                if (diff < Duration(days: int.parse(widget.minHrs))) {
                  print("not valid");
                  print("***********");
                  Fluttertoast.showToast(
                      msg: 'Please choose grater date range'.tr);
                } else {
                  List condition = [];
                        ...
                        child: SfDateRangePicker(
                          ...
                          monthViewSettings: DateRangePickerMonthViewSettings(
                            ...
                            blackoutDates: [
                              ...
                              //         int.parse(bookRangeModal.bookeddate[i].returnDate.day.toString())),
                              for (int i = 0;
                                  i < bookRangeModal.bookeddate.length;
                                  i++)
                                for (int a = 0;
                                    a <=
                                        DateTime.parse(bookRangeModal
                                                .bookeddate[i].returnDate
                                                .toString())
                                            .difference(DateTime.parse(
                                                bookRangeModal
                                                    .bookeddate[i].pickupDate
                                                    .toString()))
                                            .inDays;
                                    a++)
                                  DateTime.parse(bookRangeModal
                          ...
                          onSelectionChanged:
                            ...
                            setState(() {
                              sdate = dateRangePickerSelectionChangedArgs
                                  .value.startDate;
                              edate = dateRangePickerSelectionChangedArgs
                                  .value.endDate;
                            });
                          },
                        ),
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    borderRadius: BorderRadius.circular(13),
                                    border:
                                        Border.all(color: notifire.getCarColor),
                                  ),
                                  child: Row(
                                    children: [
                                      SizedBox(width: 15),
